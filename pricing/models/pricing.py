...
class Season(models.Model):
    ...
    def clean(self):
        ...
        if self.version:
            overlapping = overlapping.filter(version=self.version)
        if self.pk:
            overlapping = overlapping.exclude(pk=self.pk)

        if overlapping.exists():
            names = ', '.join(s.name for s in overlapping[:3])
            raise ValidationError(f"Dates overlap with: {names}")
    ...
    def get_or_create_default(cls, hotel):
        ...
        if created:
            for label, min_d, max_d, order in [
                ('0-6 Days', 0, 6, 0), ('7-29 Days', 7, 29, 1),
                ('30-59 Days', 30, 59, 2), ('60-89 Days', 60, 89, 3),
                ('90+ Days', 90, None, 4),
            ]:
