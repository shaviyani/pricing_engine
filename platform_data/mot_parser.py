...
class MoTReportParser:
    ...
    def parse_pdf(self, file_path):
        ...
        elif num_pages >= 3 and 'Table 1' in (pdf.pages[1].extract_text() or ''):
            return self._parse_monthly_report(pdf, file_path.name)
        else:
            # Try to detect from content (case-insensitive)
            all_text = '\n'.join(p.extract_text() or '' for p in pdf.pages[:3])
            all_text_lower = all_text.lower()
            if 'tourist arrivals by nationality' in all_text_lower:
                return self._parse_monthly_report(pdf, file_path.name)
            elif 'top 10 markets' in all_text_lower:
                return self._parse_daily_update(pdf, all_text, file_path.name)
            else:
    ...
    def _parse_monthly_report(self, pdf, filename):
        ...
        for page_idx in range(1, min(len(pdf.pages), 5)):
            page_text = pdf.pages[page_idx].extract_text() or ''
            
            # Stop if we've passed Table 1 entirely
            if 'Table 2' in page_text and not countries:
                # Table 2 without having found Table 1 data — skip
                continue
            if 'Table 2' in page_text and countries:
                # Reached Table 2, take only the part before it
                page_text = page_text.split('Table 2')[0]
            if 'Table 4' in page_text or 'Table 5' in page_text or 'KEY INDICATORS' in page_text:
                break
        ...
        key_indicators = {}
        for page_idx in range(4, min(len(pdf.pages), 8)):
            page_text = pdf.pages[page_idx].extract_text() or ''
            if 'KEY INDICATORS' in page_text or 'OCCUPANCY RATE' in page_text:
    ...
    def _parse_nationality_table(self, text, report_year):
        """
        Parse country rows from Table 1 text.
    ...
    def _parse_key_indicators(self, text, report_year):
        """Extract key indicators from Table 5."""
        indicators = {}
...
def import_mot_report(file_path, country_code='MV', user=None):
    ...
    """
    from platform_data.models import MarketArrivalData, PlatformFileImport
